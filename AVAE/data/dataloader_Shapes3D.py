...
class dataloader_shapes3d():
    def __init__(self, dataset_name, kde_samples, batch_size=100):

        if 'Shapes3D' in dataset_name:
            self.data_dir = '/home/sci/surojit/Research/Data/Shapes3D'
            self.data_path = os.path.join(self.data_dir, 'imgs_train.npz')
            self.imgs_train = copy.deepcopy(np.load(self.data_path)['arr_0'])
            self.imgs_train = (self.imgs_train.astype(np.float32))/255.0
