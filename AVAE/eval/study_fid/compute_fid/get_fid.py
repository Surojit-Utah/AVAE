            ...
            if shape._dims is not None:
                ...
                for j, s in enumerate(shape):
                    if s == 1 and j == 0:
                        ...
                        new_shape.append(s)
                o.__dict__['_shape_val'] = tf1.TensorShape(new_shape)
...
def calculate_fid_given_paths(gen_samples, path, inception_path, gen_samples_np=True, low_profile=False):
    ...
    with tf1.Session(config=tf1.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        # celeba, cifar10
        if gen_samples_np:
            m1, s1 = calculate_activation_statistics(gen_samples, sess)
        # mnist
        else:
            m1, s1 = _handle_path(gen_samples, sess, low_profile=low_profile)
        m2, s2 = _handle_path(path, sess, low_profile=low_profile)
        fid_value = calculate_frechet_distance(m1, s1, m2, s2)
        return fid_value


if __name__=="__main__":
    generated_image_array_dir = '/home/sci/surojit/Research/Non_Stationary_KDE/GENs_Keras_TF2/Initial_Results/MNIST/Validation_error/Square_root/Generated_Samples_FID/Output/Generated_Images_8'
    generated_image_array_path = os.path.join(generated_image_array_dir, "generated_images.npy")
    generated_image = np.load(generated_image_array_path)

    fidstat_basedir = '/home/sci/surojit/Research/Non_Stationary_KDE/GENs_Keras_TF2/Github/fid_stats_data'
    fid_stat_path = os.path.join(fidstat_basedir, 'fid_stats_mnist.npz')

    model_fid_score = calculate_fid_given_paths(generated_image, fid_stat_path, None)
    print(model_fid_score)