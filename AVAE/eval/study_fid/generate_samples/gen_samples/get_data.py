...
def sample_kde_n_train_data(x_train, kde_samples):
    ...
    set_seed()
    KDE_samples_indices = np.random.choice(x_train.shape[0], kde_samples, replace=False).tolist()
    Train_samples_indices = list(set(np.arange(x_train.shape[0]).tolist()).difference(set(KDE_samples_indices)))
    ...
    x_train_kde = x_train[KDE_samples_indices]
...
def load_data(dataset_name):
    ...
    if 'CelebA' in dataset_name:
        # Use environment variable for CelebA data directory
        data_dir = os.environ.get('CELEBA_DATA_DIR')
        if data_dir is None:
            raise ValueError(
                "CelebA dataset path not configured. Set CELEBA_DATA_DIR environment variable.\n"
                "Example: export CELEBA_DATA_DIR=/path/to/celeba/data"
            )
...
def get_data_for_encoded_statistics(dataset_name, fid_samples):
